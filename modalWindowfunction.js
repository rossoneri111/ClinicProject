...
function activateCreateCardBtn() {
    ...
    if (!cardiologist.classList.contains('d-none')) {
        const cardiologistInputs = document.querySelectorAll('.cardiologist-option__value');
        const cardiologistSelect = document.querySelector('.cardiologist-option__select');
        if ([...cardiologistInputs].every(input => input.value !== '') && cardiologistSelect.value !== '') {
            document.querySelector('.btn-create-visit__card').classList.remove('d-none');
        } else {
    ...
    if (!dentist.classList.contains('d-none')) {
        ...
        if ([...dentistInputs].every(input => input.value !== '') && dentistSelect.value !== '') {
            document.querySelector('.btn-create-visit__card').classList.remove('d-none');
        } else {
    ...
    if (!therapist.classList.contains('d-none')) {
        const therapistInputs = document.querySelectorAll('.therapist-option__value');
        const therapistSelect = document.querySelector('.therapist-option__select');
        if ([...therapistInputs].every(input => input.value !== '') && therapistSelect.value !== '') {
            document.querySelector('.btn-create-visit__card').classList.remove('d-none');
        } else {
